function [TargetSNR, TargetPhase, TargetSNRsigned, tSNR]= calcSNRmv6(X,Xraw,TargetFrequency,BinSize,Thresh,histBin)
...
%        2.TargetFrequency: the frequency bin (ordinal number) of the target frequency
...
%          noise.
%        4.Thresh: a threshold value (e.g., 3.719). The distribution of the phase values of the
%          voxels above this threshold value are used to define
...
%gap of 1
gap=1;
NoiseFs=[(TargetFrequency-BinSize/2-gap):(TargetFrequency-1-gap) (TargetFrequency+1+gap):(TargetFrequency+BinSize/2+gap)];
...
while 1
[n,x]=hist(TargetPhaseP(TargetSNR(TargetPhase>0)>Thresh),histBin);
xcenter=x(n==max(n));
if length(xcenter)>1
    histBin=histBin+1;
else
